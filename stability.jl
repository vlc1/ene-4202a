...
rk1!(res, x, y, τ, f, t) = res .= x - y - τ * f(t, y)
...
On peut montrer de plus que pour les schémas explicites de Runge-Kutta d'ordre ``p``, la raison ``\sigma`` peut s'écrire sous la forme d'un développement limité de ``\exp`` à l'ordre ``p``,
```math
