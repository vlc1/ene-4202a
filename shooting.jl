...
1. Réécrire l'équation scalaire ci-dessus sous la forme d'un système de deux EDO couplées. Implémenter ce modèle dans la fonction `source` ci-dessous.
...
shooting(midpoint!, source, 0.01, 1.0, bc!, [2.0; 1.0])
...
# ╔═╡ ab5012e6-344b-4d84-a40a-0a1c52d267bc
function foo(t, y)
	x = similar(y)
	x[1] = y[2]
	x[2] = 0
...
# ╔═╡ 254e12ad-2c97-4f1c-a8d1-96549cbc30a2
function bar!(res, left, right)
	res[1] = left[1]
	res[2] = right[1] - left[3] - 0.5
	res[3] = 2right[2] + 3left[4]
	res[4] = right[3] - left[5]
	res[5] = 3right[4] + 4left[6]
	res[6] = right[5] - 1
	nothing
end

# ╔═╡ 8e8b385b-df11-4e76-adde-dba3af07ce25
begin
	local T, Y = shooting(midpoint!, foo, 0.01, 0.5, bar!, [0.0; 1.0; 1.0; 0.0; 0.0; 1.0])
	local fig = plot(xlim = (0, 1.5))
	scatter!(fig, T, first.(Y), label = "T₁")
	scatter!(fig, T .+ 0.5, (x -> getindex(x, 3)).(Y), label = "T₂")
	scatter!(fig, T .+ 1.0, (x -> getindex(x, 5)).(Y), label = "T₃")
end
