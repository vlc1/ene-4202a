...
# ╔═╡ 4d2c77bf-fd1b-4b68-9301-c1184e76ebff
using Plots
...
# ╔═╡ 2daa2f74-48c7-44ef-9014-a0ca0e541a2c
ℯ

# ╔═╡ bea20ce9-88ff-45e4-bb5a-494c4d2d19c2
# Réponse 1 (indice: \euler + TAB)
function kepler!(res, x)
	res[1] = 10 - x[1] + ℯ * sin(x[1])
	nothing
end

# ╔═╡ ef9fb069-4941-4288-91d2-fc4a988dc71b
begin
	local x = getproperty(nlsolve(kepler!, [10.0]), :zero)
	y = similar(x)
	kepler!(y, x)
	y
end

# ╔═╡ 08f732c7-c520-4b70-9949-e9c76c5a54e8
kepler(x) = 10 - x + ℯ * sin(x)

# ╔═╡ f1cc64ad-8b3f-4ecb-912b-c422ccafbc11
begin
	local fig = plot(xlim = (-10, 10))
	plot!(fig, kepler)
end
...
function system!(res, x)
	res[1] = x[1] + x[2] + x[3] ^ 2 - 12
	res[2] = x[1] ^ 2 - x[2] + x[3] - 2
	res[3] = 2x[1] - x[2] ^ 2 + x[3] - 1
	nothing
end

# ╔═╡ 27accb94-d92f-4da7-be12-6ad94857998b
x = getproperty(nlsolve(system!, [0.0; 0.0; 0.0]), :zero)
